...
namespace WpfMailSenderClient
{
    public partial class MailBox : Window
    {
        ...
        private void btnSendLater_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ObservableCollection<ListViewItemScheduler> schedulers = (DataContext as ViewModel.MailBoxViewModel).SendMails;
                ...
                foreach (ListViewItemScheduler scheduler in schedulers)
        ...
        private async void btnSendNow_Click(object sender, RoutedEventArgs e)
        {
            MessageBox msgBox;
            try
            {
                ...
                foreach (Emails mail in (DataContext as ViewModel.MailBoxViewModel).EmailsView)
                    await mailSender.SendMail(mail.Value);
        ...
        {
            string path = "..//..//..//mailSendersList.docx";
            WordExport export = new WordExport(path);
            ObservableCollection<Emails> emails = (DataContext as ViewModel.MailBoxViewModel).Emails;
            IEnumerator<Emails> sortedEmails = (from entry in emails orderby entry.Name ascending select entry).GetEnumerator();

            string[] recipietns = new string[emails.Count];
            for (int i = 0; sortedEmails.MoveNext(); i++)
                recipietns[i] = sortedEmails.Current.Name + ", email: " + sortedEmails.Current.Value;

            TextRange tr = new TextRange(rtbReportView.Document.ContentStart, rtbReportView.Document.ContentEnd);
            foreach (string str in recipietns)
                tr.Text += str + "\r";
            
            export.Export("Список получателей", recipietns);
        }
    }
}
